...
void ajouterEnTete (unSnake * snake, int ligne, int colonne,int * aMange, int * fail) {
  ...
  if(!*fail) {
    ...
    //*aMange = (grille[snake->teteSnake->ligne][snake->teteSnake->colonne] == DU_MANGER1) ? 1 : 0;
    if (grille[snake->teteSnake->ligne][snake->teteSnake->colonne] == DU_MANGER1){
       *aMange = 1; 
...
void genererDuManger(char ** grille) {
  ...
  while (!done){
    ...
    if(grille[ligne][colonne] == CASE_VIDE) {
      grille[ligne][colonne] = DU_MANGER1;
    }
    done = 1;
    ...
    colonne = rand() % (nbColonnes-1);
    if(grille[ligne][colonne] == CASE_VIDE) {
      grille[ligne][colonne] = DU_MANGER2;
    
    }
    ligne = rand() % (nbLignes-1);
    colonne = rand() % (nbColonnes-1);
    if(grille[ligne][colonne] == CASE_VIDE) {
      grille[ligne][colonne] = DU_MANGER3;
      done = 1; */
    }
  } 
}
...
void printFail() {
  printw("Bravo ! Votre score est de : %d \n", score);
...
int main (int argc, char * argv []) {
  ...
  if (argc > 1) {
    ...
    while ((niveau !=1) && (niveau !=2) && (niveau !=3) && (niveau !=4) && (niveau !=5) && (niveau !=6) && (niveau !=7) && (niveau !=8) && (niveau !=9)){
     printf("\nChoisisez un niveau entre 1 et 9 : ");
     scanf("%d",&niveau);
    }
   
  switch (niveau){
    case 1: vitesse = 10; break;
    case 2: vitesse = 50; break;
    case 3: vitesse = 80; break;
    case 4: vitesse = 40; break;
    case 5: vitesse = 50; break;
    case 6: vitesse = 60; break;
    case 7: vitesse = 70; break;
    case 8: vitesse = 80; break;
    case 9: vitesse = 90; break;
    default : 
    vitesse = 10;
  ...
  */
  ...
  initGrille();
  //appel 
  //save (grille, nbLignes, nbColonnes);
  //  
...
void save (int grille[][nbColonnes], int nbLignes, int nbColonnes)
{
  FILE *f ; 
  f = fopen ("snake.txt", "w");   
  int i, j; 
 ...
 void load (int nbLignes, int nbColonnes)
{
  FILE *f ; 
  f = fopen ("snake.txt", "r"); 
  int i, j; 
  char * grille = malloc(nbLignes * sizeof(char *));
